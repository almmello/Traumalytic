...
def criar_boxplot_ptci(data):
    DataLoader.calculos_ptci(data)  # Preparação dos dados

    # Filtrando os dados para remover valores menores que 3
    escores_filtrados = data[data['PTCI_Total'] > 3]

    plt.figure(figsize=(10, 6))
    boxplot = plt.boxplot(escores_filtrados['PTCI_Total'], vert=True, patch_artist=True)  # Boxplot vertical
    plt.title('Boxplot dos Escores Totais do PTCI')
    plt.ylabel('Escore do PTCI')
    plt.grid(True)
